...
const checkDraw = (numCurrentPlayerMoves) => {
  const totalNumAvailMoves =
    configAttributes.num_rows * configAttributes.num_columns;
...
const doWeHaveAWinner = (moves, player, board) => {
  const goalTest = (state, board) => {
    const diagonalCheck = (board, player) => {
      const conditionCheck = (curRow, curCol) =>
        curRow >= 0 &&
        curRow < configAttributes.num_rows &&
        curCol >= 0 &&
        curCol < configAttributes.num_columns &&
      ...
      return state.some(
        (cell) =>
          mainDiagonalCheck(cell[0], cell[1]) ||
          subDiagonalCheck(cell[0], cell[1])
      );
    };
    ...
    const winConditionCheck = (winCondition, mostOccRowOrCol, direction) => {
      const win = state.reduce((connected, curCell) => {
        ...
        const rowOrColToCheck =
          direction === 'horizontal' ? curCellRow : curCellCol;
        const rowOrColToSlide =
          direction === 'horizontal' ? curCellCol : curCellRow;
        if (
          rowOrColToCheck === mostOccRowOrCol &&
          winCondition[rowOrColToSlide] != null
        ) {
    ...
    const verticalCheck = (mostOccCol, occurrences) => {
      ...
      const conditionOne = Object.fromEntries(
        Array(5)
          .fill()
          .map((_, index) => [index, index])
      );
      // a list of horizontal index from 1 to 6
      const conditionTwo = Object.fromEntries(
        Array(5)
          .fill()
          .map((_, index) => [index + 1, index + 1])
      );

      const winsInConditionOne = winConditionCheck(
        conditionOne,
        mostOccCol,
        'vertical'
      );
      if (winsInConditionOne) return true;

      const winsInConditionTwo = winConditionCheck(
        conditionTwo,
        mostOccCol,
        'vertical'
      );
    ...
    const mostOccIndex = () => {
      ...
      const find = (obj) => {
        ...
        // largest value
        const mostOccurred = occurrences.reduce(
          (most, occ, idx) =>
            occ > most[1] ? [cells[idx], occ] : [most[0], most[1]],
    ...
    const { mostOccurredRow, rowOccurrence, mostOccurredCol, colOccurrence } =
      mostOccIndex();

    if (
      verticalCheck(mostOccurredCol, colOccurrence) ||
      horizontalCheck(mostOccurredRow, rowOccurrence) ||
      diagonalCheck(board, player)
    )
      return true;
  ...
  const mapStateCellsToBoardCells = (state) => {
    return state.map((cell) => {
      ...
      const [boardRowIdx, boardColIdx] = mapSectionCellToBoardCell(
        rowIdx,
        colIdx,
        sectionIdx
      );
...
const sectionsToBoard = (prevBoard) => {
  ...
  prevBoard.map((section, sectionIdx) =>
    section.map((row, rowIdx) =>
      row.map((cell, colIdx) => {
        const [boardRow, boardCell] = mapSectionCellToBoardCell(
          rowIdx,
          colIdx,
          sectionIdx
        );
...
export default function Board(props) {
  ...
  const getCurrentPlayer = () => (nextColor === 'black' ? 0 : 1);
  ...
  function onClickCallback(colIdx, rowIdx, sectionIdx) {
    ...
    if (board[activeSectionIdx][rowIdx][colIdx].isOccupied) return;
  ...
  const calcSectionWidth = (boardWidth, sectionIdx) => {
    ...
    if (sectionIdx % 2 === 0) {
      return {
        baseWidth: (boardWidth - sectionGap - sectionBorder - boardBorder) / 2,
        marginRight: sectionGap,
      };
    }
    return {
      baseWidth: (boardWidth - sectionGap - sectionBorder - boardBorder) / 2,
  ...
  const calcSectionHeight = (boardHeight, sectionIdx) => {
    ...
    if (sectionIdx < configAttributes.num_sections / 2) {
      return {
        baseHeight:
          (boardHeight - sectionGap - sectionBorder - boardBorder) / 2,
        marginBottom: sectionGap,
      };
    }
    return {
      baseHeight: (boardHeight - sectionGap - sectionBorder - boardBorder) / 2,
      marginBottom: 0,
    };
  };
  ...
  const rotateSection = (sectionIdx, option) => {
    if (option === 'skip' || isRotateSkippable(board, sectionIdx)) {
      ...
      if (
        currentPlayerMoves.length >= 5 &&
        doWeHaveAWinner(currentPlayerMoves, nextColor, board)
      ) {
        dispatch({
          type: 'UPDATE WINNER',
        ...
        dispatch({
          ...
          newState: {
            ...
            topMessage:
              rotationNoEffectMessage(sectionIdx + 1) +
              ' ' +
              nextPhaseMessage(newNextColor, states[newPhase]),
    ...
    const activeCellsIdx = activeSection.map((row) =>
      row.map((_, cellIdx) => cellIdx)
    );

    // create new active section with color set to white
    const newActiveSection = activeSection.map((row) =>
      row.map((cell) => {
        const newCell = { ...cell };
        newCell['color'] = 'white';
        return newCell;
    ...
    const blueMovesToRotate = Object.fromEntries(
      newMoves[0]
        .map((move, index) => {
    ...
    const redMovesToRotate = Object.fromEntries(
      newMoves[1]
        .map((move, index) => {
          if (move[0] === sectionIdx) {
            return [[...move], index];
    ...
    activeCellsIdx.forEach((row, rowIdx) =>
      row.forEach((colIdx, _) => {
        ...
        newActiveSection[newRowIdx][newColIdx] =
          activeSection[oldRowIdx][oldColIdx];

        const newBlueMoveIdx =
          blueMovesToRotate[[sectionIdx, oldRowIdx, oldColIdx]];
        const newRedMoveIdx =
          redMovesToRotate[[sectionIdx, oldRowIdx, oldColIdx]];
    ...
    if (
      currentPlayerMoves.length >= 5 &&
      doWeHaveAWinner(currentPlayerMoves, nextColor, newBoard)
    ) {
      dispatch({
        type: 'UPDATE WINNER',
        winnerColor: nextColor,
    ...
    const topMessage =
      reportLastActionMessage(nextColor, 'rotate', sectionIdx + 1) +
      '\n' +
  ...
  const undoClick = () => {
    ...
    const [lastSectionIdx, lastRowIdx, lastColIdx] =
      prevMoves[currentPlayer].pop();
    const prevBoard = board.map((section, sectionIdx) =>
      section.map((row, rowIdx) =>
        row.map((cell, colIdx) => {
          if (
  ...
  const sectionWidths = Array(4)
    .fill()
    .map((_, index) => calcSectionWidth(boardwidth, index));
  const sectionHeights = Array(4)
    .fill()
    .map((_, index) => calcSectionHeight(boardHeight, index));
    ...
    <Fragment>
      ...
      >
        ...
        <Modal
          ...
          onModalClickCallback={onModalClickCallback}
        />
      </Stack>
    </Fragment>
  );
